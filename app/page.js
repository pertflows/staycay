      ...
      <section className="section-gap section-padding bg-brand-cream">
        <RevealGroup className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-4">
            <div>
              <span className="reveal eyebrow">Follow Along</span>
              <h2 className="reveal reveal-delay-1 font-display text-display-lg text-brand-navy mt-3"
                  style={{ fontWeight: 400 }}>
                @StayCay
              </h2>
            ...
            <a
              href="https://instagram.com/thestaycay"
              target="_blank"
              rel="noopener noreferrer"
              className="reveal reveal-delay-2 flex items-center gap-2 text-brand-muted hover:text-brand-coral
                         transition-colors duration-300 font-body text-xs uppercase tracking-[0.15em] shrink-0"
            >
          ...
          {/* 8-cell grid — swap with live IG feed once API token is connected */}
          <div className="reveal reveal-delay-2 grid grid-cols-2 md:grid-cols-4 gap-3">
            {Array.from({ length: 8 }).map((_, i) => (
              <div
                key={i}
                className="aspect-square bg-brand-sand/60 border border-brand-sand
                           flex items-center justify-center group cursor-pointer
                           hover:border-brand-gold/40 transition-all duration-300 relative overflow-hidden"
              >
                {/* Ambient gradient on hover */}
                <div className="absolute inset-0 bg-gradient-to-br from-brand-gold/0 to-brand-gold/5
                                opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
                <svg
                  className="w-7 h-7 text-brand-muted/30 group-hover:text-brand-gold/50 transition-colors duration-300 relative z-10"
          ...
          <p className="text-center text-brand-muted/50 text-xs uppercase tracking-[0.15em] mt-6 reveal">
            Live feed connects once Instagram API token is provided
          </p>
        </RevealGroup>
      </section>
